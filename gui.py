...
class GUI(object):
    ...
    def command(self, event):
        ...
        if cmddic['instruction'] == 'cleanmem':
            ...
            self.panel.insert(INSERT, '\nOut [' + str(int(self.outputline)) + ']: ' + str(parameter.parameterdic[cmddic['varname']].name) + ' = ' + str(parameter.parameterdic[cmddic['varname']].matrix))
            self.outputline += len(cmddic['varvalue'])
            self.currentline += len(cmddic['varvalue'])+1
            ...
            self.panel.tag_config('error_tag', foreground='red')
            self.panel.insert(INSERT, '\nPlease follow the format when you assign a polynomial: "varname(symbol)" ', 'error_tag')
            self.outputline += 1
            self.currentline += 2
    ...
    def doplot(self, event):
        xlowerbound1 = int(self.xlowerboundentry1.get())
        xupperbound1 = int(self.xupperboundentry1.get()) + 1
        formula1 = self.formulaentry1.get()
        if self.formulaentry2 and self.xlowerboundentry2 and self.xupperboundentry2:
            xlowerbound2 = int(self.xlowerboundentry2.get())
            xupperbound2 = int(self.xupperboundentry2.get()) + 1
            formula2 = self.formulaentry2.get()
            function.plot(xlowerbound1, xupperbound1, formula1, xlowerbound2, xupperbound2, formula2)
        else:
            function.plot(xlowerbound1, xupperbound1, formula1)
        self.draw.destroy()

    def checkplot(self):
        isplot = self.checksameplotvar.get()
        if function.isplots(isplot):
            self.checksameplotvar.set(0)
        else:
            self.checksameplotvar.set(1)
            self.formulalabel2 = Label(self.draw, text='  方程式: ')
            self.xlabel2 = Label(self.draw, text='  x的範圍: ')

            Label(self.draw, text=' ').grid(row=8, column=0, sticky=E)
            self.formulalabel2.grid(row=10, column=0, sticky=E)
            self.xlabel2.grid(row=12, column=0, sticky=E)
            Label(self.draw, text='~').grid(row=12, column=2, sticky=E)

            self.formulaentry2 = Entry(self.draw, width=13)
            self.xlowerboundentry2 = Entry(self.draw, width=13)
            self.xupperboundentry2 = Entry(self.draw, width=13)

            self.formulaentry2.grid(row=10, column=1, sticky=E)
            self.xlowerboundentry2.grid(row=12, column=1, sticky=E)
            self.xupperboundentry2.grid(row=12, column=3, sticky=E)
    ...
    def plot(self):
        ...
        self.formulalabel1 = Label(self.draw, text='  方程式: ')
        self.xlabel1 = Label(self.draw, text='  x的範圍: ')
        # self.kind = Label(self.draw, text='  樣式: ')

        Label(self.draw, text = ' ').grid(row = 0, column = 0, sticky = E)
        self.formulalabel1.grid(row = 2, column = 0, sticky = E)
        self.xlabel1.grid(row = 4, column = 0, sticky = E)
        ...
        self.formulaentry1 = Entry(self.draw, width = 13)
        self.xlowerboundentry1 = Entry(self.draw, width = 13)
        self.xupperboundentry1 = Entry(self.draw, width = 13)
        # self.ylowerboundentry = Entry(self.draw, width = 13)
        # self.yupperboundentry = Entry(self.draw, width = 13)

        self.formulaentry1.grid(row=2, column=1, sticky=E)
        self.xlowerboundentry1.grid(row=4, column=1, sticky=E)
        self.xupperboundentry1.grid(row=4, column=3, sticky=E)
        ...
        self.checksameplotvar = IntVar()
        self.checksameplot = Checkbutton(self.draw, text='同張圖', variable=self.checksameplotvar, command=self.checkplot)
        self.checksameplot.grid(row=6, column=1, sticky=E)
